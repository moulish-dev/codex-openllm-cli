...
// agent-loop.ts — Ollama-only simplified version
...
export class AgentLoop {
  ...
  async run(input: Array<ResponseInputItem>): Promise<void> {
    try {
      ...
      const response = await client.prompt(promptText);
      ...
      this.onItem({
        ...
        content: [
          {
            type: "input_text",
            text: `⚠️  Failed to contact Ollama. ${error}`,
          },
        ],
      });
      this.onLoading(false);
...
// NOTE: This file assumes you define your own ResponseItem and ResponseInputItem
// types based on your local usage. Replace with OpenAI-compatible types if needed.
